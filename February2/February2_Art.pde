...
float translate;  // variable to translate the X and Y position of the rectangles
...
void createGrid() {
  for (int gridX = 0; gridX < tileCount; gridX++) {  // loop through each X position
    for (int gridY = 0; gridY < tileCount; gridY++) {  // loop through each Y position
      ...
      translate = width/tileCount/2;  // to translate X and Y position by approximately half the rectangle size 
      rect(posX + translate, posY + translate, (width/tileCount) + randSize, (height/tileCount) + randSize);
    }
  }
}
